...
**环境要求：** JDK 17+ · Maven 3.8+ · LangChain4j 1.15.x 或 Spring AI 1.0.0+ (2.0.x)
...
开发者代码
     ...
     ▼
┌──────────────────────────────────────────┐
│ jswarm-adapter-langchain4j               │  LangChain4j 适配层
├──────────────────────────────────────────┤
│ jswarm-adapter-spring-ai                 │  Spring AI 适配层
├──────────────────────────────────────────┤
│ jswarm-core                              │  Swarm / Agent / SwarmContext (纯 JDK)
└──────────────────────────────────────────┘
     │
     ▼
...
| `jswarm-adapter-langchain4j` | LangChain4j 适配层：JAgent 运行时、SwarmRunner、工具桥接、过滤分发 |
| `jswarm-adapter-spring-ai` | Spring AI 适配层：JAgent 运行时、SwarmRunner（支持同步与流式）、自动配置、Advisors 桥接 |
...
- `JAgent.builder()`：构建 Agent，支持钩子 lambda 和 Spring AI `@Tool`（`ToolCallback`）桥接
- `JAgent.fromTools()` / `fromAiService()`：桥接已有的 Spring AI 工具回调
- `JAgent.decorate()`：装饰器模式叠加钩子
- `SwarmRunner.runStreaming()`：流式输出运行，支持完整的事件生命周期监听（`SwarmEvent`）
- `JswarmAutoConfiguration`：Spring Boot 自动装配，支持 Spring 属性配置（如 `jswarm.max-turns`）
- `SwarmLoggingAdvisor` / `SwarmMetricsAdvisor`：基于 Spring AI Advisor 的日志与指标观测
...
Examples 模块依赖真实 LLM，CI 通常跳过；单元测试集中在 core 模块、langchain4j 适配层与 spring-ai 适配层模块。
...
- 基于 Spring AI 的流式事件模型与 SSE 演示
- Spring Boot 自动装配（仅限 Spring AI 适配层）

**规划中**

- 更多模型与框架适配器
- 统一的流式事件接口与 LangChain4j 流式支持
